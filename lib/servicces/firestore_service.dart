...
class FirestoreService {
  ...
  static String? calcularRangoEdad(String fechaNacimiento) {
    try {
      final parts = fechaNacimiento.trim().split('-');
      ...
      if (nacimiento.isAfter(hoy)) return null;
  ...
  static Future<Map<String, int>> migrarRangosEdadNinos() async {
    ...
    for (final doc in todos.docs) {
      final data = doc.data();
      final fecha = (data['fecha_nacimiento'] as String? ?? '').trim();
  ...
  static Future<void> actualizarPadre(String uid, Map<String, dynamic> datos) async {
  ...
  static Future<void> guardarUbicacionPadre(String uid, double lat, double lng) async {
  ...
  static Future<bool> validarPasswordNino(String ninoId, String password) async {
  ...
  static Future<Map<String, dynamic>> vincularNinoPadre({
    ...
    final q = await _db
        .collection('ninos')
        .where('codigo_vinculacion', isEqualTo: codigoVinculacion.trim().toUpperCase())
        .limit(1)
        .get();
  ...
  static Future<List<Map<String, dynamic>>> listarNinosPadre(String padreId) async {
    final q = await _db
        .collection('ninos')
  ...
  static Future<List<Map<String, dynamic>>> listarNinosPorRango(String padreId, String rangoId) async {
    final q = await _db
        .collection('ninos')
        .where('id_padre', isEqualTo: padreId)
  ...
  static Future<void> guardarUbicacionNino(String ninoId, double lat, double lng) async {
  ...
  static Future<List<Map<String, dynamic>>> obtenerCategoriasNino(String ninoId) async {
    final doc = await _db.collection('ninos').doc(ninoId).get();
    if (!doc.exists) return [];
    final data = doc.data()!;
    final List ids = (data['categorias_permitidas'] as List?) ?? [];
    return ids
        .map<Map<String, dynamic>>((id) => _catalogoCategorias[id.toString()] ?? {})
        .where((c) => c.isNotEmpty)
        .toList();
  }

  static Future<void> guardarCategoriasNino(String ninoId, List<String> categoriaIds) async {
    await _db
        .collection('ninos')
        .doc(ninoId)
  ...
  static Future<List<Map<String, dynamic>>> obtenerVideosCatalogo({
    ...
    return q.docs
        ...
        .map((data) => {
              'video_id': (data['video_id'] as String? ?? '').trim(),
              'titulo': data['titulo'] ?? '',
  ...
  static Future<List<Map<String, dynamic>>> obtenerVideosPorCategoria(String categoriaId) async {
    final q = await _db
        .collection('videos')
  ...
  static Map<String, String>? obtenerCanalDefault(String catId) => _catalogoCanales[catId];

  static Future<List<Map<String, dynamic>>> obtenerCanalesCustom(String padreId, String catId) async {
    final q = await _db
        .collection('canales')
        .where('id_padre', isEqualTo: padreId)
  ...
  static Future<List<Map<String, dynamic>>> obtenerTodosCanalesCustom(String padreId) async {
    final q = await _db
        .collection('canales')
        .where('id_padre', isEqualTo: padreId)
        .where('activo', isEqualTo: true)
  ...
  static Future<List<Map<String, dynamic>>> obtenerYoutubersNino(String ninoId) async {
    final q = await _db
        .collection('youtubers_nino')
        .where('id_nino', isEqualTo: ninoId)
        .where('activo', isEqualTo: true)
        .get();
    return q.docs.map((d) => {'id': d.id, ...d.data()}).toList();
  }

  static Future<void> agregarYoutuberNino({
    required String ninoId,
    required String channelUrl,
    required String nombreCanal,
  }) async {
    final urlNormalizada = channelUrl.trim().toLowerCase();
    final existente = await _db
        .collection('youtubers_nino')
        .where('id_nino', isEqualTo: ninoId)
        .where('channel_url_normalized', isEqualTo: urlNormalizada)
        .limit(1)
        .get();

    if (existente.docs.isNotEmpty) {
      await existente.docs.first.reference.set({
        'nombre_canal': nombreCanal.trim(),
        'channel_url': channelUrl.trim(),
        'channel_url_normalized': urlNormalizada,
        'activo': true,
    ...
    await _db.collection('youtubers_nino').add({
      'id_nino': ninoId,
      'channel_url': channelUrl.trim(),
      'channel_url_normalized': urlNormalizada,
      'nombre_canal': nombreCanal.trim(),
  ...
  static Future<void> eliminarYoutuberNino(String docId) async {
    await _db.collection('youtubers_nino').doc(docId).delete();
  }
  ...
  static Future<void> registrarTiempoUso({
    ...
    if (idUsuario.trim().isEmpty) return;
